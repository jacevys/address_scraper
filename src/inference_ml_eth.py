...
def inference(model_name, address, return_function):
    ...
    label_mapping_1 = {'others': 0, 'kyc': 1}
    ...
    if label_dict['judged']:
        ...
        process.terminate()
        label_dict[address] = {'exchange': '80%', 'others': '15%', 'kyc': '5%'}
        ...
        if data == {}:
           label_dict[address] = {'others': '90%', 'kyc': '5%', 'exchange': '5%'}
           label_dict['judged'] = True
...
def get_data(uri: str, username: str, password: str, database: str, address: str) -> \
    ...
    LIMIT $limit
    UNION
    MATCH (a:account {address: $address})<--(t3:internal_transaction)<-[:includes]-(t4:transaction)<-[:includes]-(b3:block)
    WITH t4.hash AS hash, b3.timestamp AS timestamp, t3.transaction_index AS token_address, t3.value AS value, 'internal_transaction' AS trx_type, 'receive' AS receive_send
    RETURN hash, timestamp, token_address, value, trx_type, receive_send
    LIMIT $limit
    UNION
    MATCH (a:account {address: $address})-->(t5:internal_transaction)<-[:includes]-(t6:transaction)<-[:includes]-(b4:block)
    WITH t6.hash AS hash, b4.timestamp AS timestamp, t5.transaction_index AS token_address, t5.value AS value, 'internal_transaction' AS trx_type, 'send' AS receive_send
    RETURN hash, timestamp, token_address, value, trx_type, receive_send
    LIMIT $limit
    }
    RETURN hash, timestamp, token_address, value, trx_type, receive_send
    ...
    '''
    MATCH (a:account {address: $address})<--(t7:token_transfer)<-[:includes]-(t8:transaction)<-[:includes]-(b5:block)
    WHERE NOT 'Y2024' IN labels(t8)
    WITH t8.hash AS hash, b5.timestamp AS timestamp, t7.token_address AS token_address, t7.value AS value, 'internal_transaction' AS trx_type, 'receive' AS receive_send
    RETURN hash, timestamp, token_address, value, trx_type, receive_send
    LIMIT $limit
    UNION
    MATCH (a:account {address: $address})<--(t9:token_transfer)<-[:includes]-(t10:transaction)<-[:includes]-(b6:block)
    WHERE NOT 'Y2024' IN labels(t10)
    WITH t10.hash AS hash, b6.timestamp AS timestamp, t9.token_address AS token_address, t9.value AS value, 'internal_transaction' AS trx_type, 'receive' AS receive_send
    RETURN hash, timestamp, token_address, value, trx_type, receive_send
    LIMIT $limit
    '''
    ...
    first_trx = float(result_sorted[-1].get('timestamp'))
    last_trx = float(result_sorted[0].get('timestamp'))
...
def classify_trx(result_sorted: list) -> Tuple[List[float], List[float]]:
    ...
    for trx in result_sorted:
        ...
        if trx.get('trx_type') == 'transaction':
            trx_value = (float(trx.get('value')) / (10 ** 18)) * 3500
...
def get_feature(address: str, queue: object, label_dict: object) -> None:
    ...
    if total_degree > 500:
        label_dict[address] = {'exchange': '90%', 'others': '5%', 'kyc': '5%'}
        label_dict['judged'] = True
...
def main(address):
    ...
    result = inference(model_name='eth_xgb_1', address=address, return_function=return_result)
